...
class ParallelSampler {
 public:
  ...
  ParallelSampler(std::vector<EdgeIDType>& _indptr,
                  std::vector<EdgeIDType>& _indices,
                  std::vector<EdgeIDType>& _eid,
                  std::vector<TimeStampType>& _ts, int _num_thread_per_worker,
                  int _num_workers, int _num_layers,
                  std::vector<int>& _num_neighbors, bool _recent,
                  bool _prop_time, int _num_history,
                  TimeStampType _window_duration)
      : indptr(_indptr),
        indices(_indices),
        eid(_eid),
        ts(_ts),
    ...
    for (int i = 0; i < num_nodes; i++) omp_init_lock(&ts_ptr_lock[i]);
...
#pragma omp parallel for schedule( \
    static, int(ceil(static_cast <float>(root_nodes.size()) / num_threads)))
    for (std::vector<NodeIDType>::size_type i = 0; i < root_nodes.size(); i++) {
      NodeIDType n = root_nodes[i];
      omp_set_lock(&(ts_ptr_lock[n]));
      for (std::vector<EdgeIDType>::size_type j = ts_ptr[slc][n];
           j < indptr[n + 1]; j++) {
        if (ts[j] > (root_ts[i] + offset - 1e-7f)) {
          if (j != ts_ptr[slc][n]) ts_ptr[slc][n] = j - 1;
          break;
        }
        if (j == indptr[n + 1] - 1) {
          ts_ptr[slc][n] = j;
  ...
  inline void add_neighbor(std::vector<NodeIDType>* _row,
                           std::vector<NodeIDType>* _col,
                           std::vector<EdgeIDType>* _eid,
                           std::vector<TimeStampType>* _ts,
                           std::vector<TimeStampType>* _dts,
                           std::vector<NodeIDType>* _nodes, EdgeIDType& k,
  ...
  inline void combine_coo(
      TemporalGraphBlock& _ret, std::vector<NodeIDType>** _row,
      std::vector<NodeIDType>** _col, std::vector<EdgeIDType>** _eid,
      std::vector<TimeStampType>** _ts, std::vector<TimeStampType>** _dts,
      std::vector<NodeIDType>** _nodes, std::vector<int>& _out_nodes) {
...
#pragma omp parallel for schedule(static, 1)
    for (int tid = 0; tid < num_threads; tid++) {
      ...
      std::transform(
          _col[tid]->begin(), _col[tid]->end(), _col[tid]->begin(),
          [&](auto& v) { return v + cum_col[tid] + num_root_nodes; });
      std::copy(_row[tid]->begin(), _row[tid]->end(),
                _ret.row.begin() + cum_col[tid]);
      std::copy(_col[tid]->begin(), _col[tid]->end(),
                _ret.col.begin() + cum_col[tid]);
      std::copy(_eid[tid]->begin(), _eid[tid]->end(),
                _ret.eid.begin() + cum_col[tid]);
      std::copy(_ts[tid]->begin(), _ts[tid]->end(),
                _ret.ts.begin() + cum_col[tid] + num_root_nodes);
      std::copy(_dts[tid]->begin(), _dts[tid]->end(),
                _ret.dts.begin() + cum_col[tid] + num_root_nodes);
      std::copy(_nodes[tid]->begin(), _nodes[tid]->end(),
                _ret.nodes.begin() + cum_col[tid] + num_root_nodes);
      delete _row[tid];
      delete _col[tid];
      delete _eid[tid];
      delete _ts[tid];
  ...
  void sample_layer(std::vector<NodeIDType>& _root_nodes,
                    std::vector<TimeStampType>& _root_ts, int neighs,
                    bool use_ptr, bool from_root) {
    ...
    if (use_ptr) update_ts_ptr(num_history, *root_nodes, *root_ts, 0);
    ...
    for (int i = 0; i < num_history; i++) {
      ...
      if ((use_ptr) && (std::abs(window_duration) > 1e-7f))
        update_ts_ptr(num_history - 1 - i, *root_nodes, *root_ts,
      ...
      int reserve_capacity =
          int(ceil((*root_nodes).size() / num_threads)) * neighs;
#pragma omp parallel
      {
...
// #pragma omp critical
//                     std::cout<<tid<<" sampling: "<<root_nodes->size()<<"
//                     "<<int(ceil((*root_nodes).size() /
//                     num_threads))<<std::endl;
#pragma omp for schedule( \
    static,               \
    int(ceil(static_cast <float>((*root_nodes).size()) / num_threads)))
        for (std::vector<NodeIDType>::size_type j = 0; j < (*root_nodes).size();
             j++) {
          NodeIDType n = (*root_nodes)[j];
          // if (tid == 16)
          //     std::cout << _out_node[tid] << " " <<j << " " << n <<
          //     std::endl;
          ...
          if (use_ptr) {
            ...
            if (num_history == 1) {
              ...
              auto e_it =
                  std::lower_bound(ts.begin() + indptr[n],
                                   ts.begin() + indptr[n + 1], nts + offset);
              e_search = std::max(long(e_it - ts.begin()), s_search);
            }
            if (tid == 0) ret[0].search_time += omp_get_wtime() - t_search_s;
          }
          ...
          if ((recent) || (e_search - s_search < neighs)) {
            ...
            for (EdgeIDType k = e_search - 1;
                 k >= std::max(s_search, e_search - neighs); k--) {
              add_neighbor(_row[tid], _col[tid], _eid[tid], _ts[tid], _dts[tid],
                           _nodes[tid], k, nts, _out_node[tid]);
            }
          } else {
            // random sampling within ptr
            for (int _i = 0; _i < neighs; _i++) {
              EdgeIDType picked =
                  s_search + rand_r(&loc_seed) % (e_search - s_search);
              add_neighbor(_row[tid], _col[tid], _eid[tid], _ts[tid], _dts[tid],
                           _nodes[tid], picked, nts, _out_node[tid]);
            }
          }
          _out_node[tid] += 1;
          if (tid == 0) ret[0].sample_time += omp_get_wtime() - t_sample_s;
        }
      }
...
PYBIND11_MODULE(sampler_core, m) {
  py::class_<TemporalGraphBlock>(m, "TemporalGraphBlock")
      ...
      .def("row",
           [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.row); })
      .def("col",
           [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.col); })
      .def("eid",
           [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.eid); })
      .def("ts", [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.ts); })
      .def("dts",
           [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.dts); })
      .def("nodes",
           [](const TemporalGraphBlock& tgb) { return vec2npy(tgb.nodes); })
      .def("dim_in", [](const TemporalGraphBlock& tgb) { return tgb.dim_in; })
      .def("dim_out", [](const TemporalGraphBlock& tgb) { return tgb.dim_out; })
      .def("tot_time",
           [](const TemporalGraphBlock& tgb) { return tgb.tot_time; })
      .def("ptr_time",
           [](const TemporalGraphBlock& tgb) { return tgb.ptr_time; })
      .def("search_time",
           [](const TemporalGraphBlock& tgb) { return tgb.search_time; })
      .def("sample_time",
           [](const TemporalGraphBlock& tgb) { return tgb.sample_time; })
      .def("coo_time",
           [](const TemporalGraphBlock& tgb) { return tgb.coo_time; });
  py::class_<ParallelSampler>(m, "ParallelSampler")
      .def(py::init<std::vector<EdgeIDType>&, std::vector<EdgeIDType>&,
                    std::vector<EdgeIDType>&, std::vector<TimeStampType>&, int,
                    int, int, std::vector<int>&, bool, bool, int,
                    TimeStampType>())
